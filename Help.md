...
3. **`terminal_putchar('\t')` 跳到下一个制表位**。每 `TAB_WIDTH`（默认 8）列一个制表位，跳过处不写入字符（原位置内容保留）。所以 `"Name\tSize"` 这类字符串能正常对齐，不会显示乱码。制表位宽度在 main.c 顶部的 `TAB_WIDTH` 宏里调（改成 4 会更紧凑）。

4. **屏幕写满后不会滚动**。超过第 25 行时，光标会被"夹住"在最后一行，继续写会覆盖最后一行（`terminal_putchar` 里有 `if (pos >= SCR_SIZE)` 的处理）。所以调试时如果发现屏幕"不动了"，可能是写满了。
...
④ 用扫描码查表得到字符（kbdus / kbdus_shift 两张表）
   ...
   ├─ 是 Tab  → 存进 input_buf 并回显（光标跳到下一个制表位）
...
1. **kbdus / kbdus_shift 是两张对照表**：普通映射表（`kbdus`）和按住 Shift 时的映射表（`kbdus_shift`）。想改按键映射（比如把 Caps Lock 映射成别的），改这两张表即可——但**不建议**，键盘映射表很容易改乱。**Tab 键**（映射表第 15 号）已经被支持：按下去输入一个 `'\t'` 字符，回显为跳到下一个制表位（8.2 节），在命令中与空格等价（17.0 节）。
...
2. **空格和 Tab 都是参数的分隔符**：`add 3 5` 里有两个空格，把字符串切成三段：`add`、`3`、`5`。按 Tab 键输入的 `'\t'` 与空格等价（`add<Tab>3<Tab>5` 效果相同）。**注意**：双引号内的 Tab 会作为内容保留（如 `echo "a<Tab>b"` 输出 a 跳制表位 b），不会被当作分隔符。
...
第 2 步：char* 参数变量 = p;
        然后 while (*p && *p != ' ' && *p != '\t') p++;   // 走到参数分隔处（空格或 Tab）
...
> 本部分是文件系统的**专属章节**：先讲清楚它是什么、有什么特点、和 Windows/Linux 有什么不同；再详解"万物皆文件"这个理论基础；最后是两份实用手册——**日常维护**和**添加新功能**，均面向只掌握 C 基础语法的新手。
>
> 相关章节索引：数据结构逐字段注释见 **§9**；修改容量上限的详细教程见 **§18**；给文件系统加接口的完整实战（rename）见 **§19**；文件系统回归测试清单见 **§39.2**。

## 28. 文件系统总览：它是什么、有什么特点

### 28.1 一句话理解

MaxZOS 的文件系统是：

> 一张**静态数组表**，把**文件和目录**统一装在一起，用**两条链表**组织成**多级目录树**，全部数据在**内存**里，**重启即丢**。

代码上它由 `fs/` 目录下的三个文件构成（`fs.h` 数据结构与接口、`fs.c` 实现、`str.c` 字符串工具），总共约 450 行——你完全有能力读懂它、改它。

先体验一下它"长什么样"。启动系统后：

```
os/> mkdir docs
os/> create docs/note "hello world"
os/> ls docs
Name	Size(Byte)
note	11
os/> cat docs/note
...
注意提示符从 `os/>` 变成了 `os/docs>`——**提示符跟随当前目录**（这是 `main.c` 里 `print_prompt()` 调用 `fs_pwd()` 实现的，见 §31.4）。

### 28.2 七大特点

下面每条都标出了"在代码的哪里"，方便你对照着看。

**特点 1：多级目录（树形结构）**
目录可以无限嵌套（实际受 64 条上限约束）。这是最近一次大改的成果——旧版文件系统是**平铺**的（只有一层"文件表"），现在支持 `mkdir a/b/c` 任意深度的路径。树形结构靠每个条目记录 `parent`（父目录）实现，见 `fs.h` 的 `file_t.parent`。

**特点 2：目录与文件共用一张条目表**
整个文件系统就是 `fs.c` 里的 `static file_t files[FS_MAX_FILES]`。一个 `file_t` 既能当文件也能当目录，靠 `type` 字段区分（`FS_TYPE_FILE` / `FS_TYPE_DIR`）。这就是"万物皆文件"的简化版（理论详解见 §30）。
- **文件**是叶子：内容存在 `content[256]` 里；
- **目录**是容器：内容就是"子链"（见特点 3），它的 `content` 字段闲置不用——这是"共用一张表"换来的小冗余，但换来的是结构极简。

**特点 3：兄弟链组织子项（不存"孩子数组"）**
每个目录的"内容"是一条链表：`first_child`（第一个孩子）→ `next_sibling`（下一个兄弟）→ `-1`（链尾）。`ls` 就是从头到尾走这条链。链表方式的代价是"找某个名字要遍历"，好处是**一个条目只占一个槽位**，不需要预留定长孩子数组（否则 64 条上限下目录深度会很受限）。遍历代码见 `fs.c` 的 `fs_list()`。

**特点 4：路径解析集中、且零拷贝**
所有 API（`fs_create`/`fs_mkdir`/`fs_read`/`fs_delete`/`fs_cd` 等）的路径逻辑都收敛在**一个函数** `fs_resolve_path()` 里（`fs.c`），其他函数不自己解析路径——要改路径语义（比如支持 `~`、支持 `//`），只动这一个函数。
解析出的路径段是**零拷贝**的：`name_out` 直接指向传入路径字符串的内部，段尾可能不是 `'\0'`（可能是 `'/'`），拷贝时必须按 `namelen` 逐字节来（`fs_add_entry` 就是这么做的，别图省事用 `strcpy`）。

**特点 5：完整错误码体系**
所有 API 返回 `fs_status_t` 枚举（`fs.h` 里 14 个错误码），命令层用 `fs_err_str()`（`main.c`）翻译成屏幕提示。错误码**只能追加在末尾、不能重排**（`fs.h` 里有注释约定），因为将来错误码可能被存储下来或用于协议。

**特点 6：API 签名冻结、存储后端可替换**
`fs.h` 公开了 8 个函数（见 §31.6 表格），签名自文件系统诞生起就没变过。`fs.c` 文件头注释明确写了设计意图：**将来要迁移到磁盘后端，只需整体替换 `fs.c` 的内部实现，命令层零改动**。`fs_init()` 里预留了挂载钩子注释（将来解析 multiboot modules / 挂载磁盘）。

**特点 7：与显示层解耦**
`fs` 模块**不认识 VGA、不认识终端**。`fs_list()` 要输出内容时，通过回调函数 `fs_out_fn` 把字符串交出去（`main.c` 里 `term_write_cb()` 负责转发到屏幕）。所以 `fs` 模块可以独立编译、独立测试，将来接串口输出也不改它。

### 28.3 设计取舍：为什么这么简单

| 真实系统有的 | MaxZOS 没有 | 为什么 |
|---|---|---|
| 权限（属主/组/权限位） | ❌ | 单用户系统，不需要 |
| 时间戳（创建/修改时间） | ❌ | 没有时钟驱动，存了也没意义 |
| 磁盘驱动、块缓存 | ❌ | 纯内存，天然"永不失速" |
| 日志、崩溃恢复 | ❌ | 重启全丢，没有"崩溃"可言 |
| 符号链接、硬链接 | ❌ | 链表模型下暂不需要 |

这些不是"还没做"，而是**刻意砍掉的**——砍掉它们，文件系统核心只剩 450 行，你才可能读懂。将来想加，每个都有清晰的位置（见 §33.4 后端替换思路）。

## 29. 和 Windows / Linux 有什么不同

### 29.1 一张表看懂三者的区别

| 维度 | Windows | Linux | MaxZOS |
|---|---|---|---|
| 存储介质 | 磁盘（NTFS/FAT） | 磁盘（ext4/xfs…）+ 内存（tmpfs） | 内存（`.bss` 静态数组） |
| 根的数量 | 每盘一个（`C:\`、`D:\`） | 一个 `/` | 一个 `/` |
| 路径分隔符 | `\` | `/` | `/` |
| 路径写法 | `C:\Users\me\a.txt` | `/home/me/a.txt` | `/docs/note` |
| 大小写敏感 | 默认不敏感 | 敏感 | 敏感 |
| 权限 | ACL / 只读属性 | 属主、组、权限位 | 无 |
| 时间戳 | 有 | 有 | 无 |
| 挂载概念 | 盘符/装入点 | 挂载点 | 无（`fs_init()` 即"挂载"） |
| 持久性 | 断电保留 | 断电保留 | **重启丢失** |
| 扩展名 | 强约定（`.exe` 决定执行） | 不依赖扩展名 | 不依赖（`create` 随便起名） |
| 特殊位置 | 回收站、`C:\Windows` | `/dev` `/proc` `/sys` | 无 |

### 29.2 与 Windows 的差异（重点）

- **没有盘符**：Windows 每个盘是一个独立的"根"（`C:\`、`D:\`…），文件系统之间互相看不见；MaxZOS 和 Linux 一样只有一个根 `/`，全系统一棵树。
- **路径用正斜杠**：`/docs/note`，不是 `C:\docs\note`。这在 C 字符串里还有个附带好处——`\` 在 C 里是转义符，用 `/` 就少一层转义烦恼。
- **扩展名没有魔法**：Windows 靠扩展名决定行为（双击 `.exe` 运行）；MaxZOS 不看扩展名，`create a.exe "hello"` 和 `create a.txt "hello"` 完全一样，`cat` 按内容读取。
- **大小写敏感**：`docs` 和 `Docs` 是两个不同的名字（Linux 亦然）。
- **没有"属性对话框"**：没有只读、隐藏、创建时间这些元数据——`file_t` 里总共就名字、类型、两个链指针、长度、内容这几个字段，一眼能看全。

### 29.3 与 Linux 的差异（重点）

- **没有权限系统**：Linux 每个文件有属主、组、权限位（`rwxr-xr-x`）；MaxZOS 任何人都能读写任何文件——因为它只有一个人。
- **没有挂载点**：Linux 的 `/home`、`/mnt/usb` 可能是不同的文件系统"贴"在目录树的某处；MaxZOS 只有一张表，不存在"另一块盘贴进来"的概念。
- **没有 `/dev`、`/proc`、`/sys`**：Linux 把设备、内核信息、硬件状态都伪装成文件给你看；MaxZOS 的屏幕、键盘、电源管理都还是"端口操作"，不是文件（这正是"万物皆文件"没做完的部分，见 §30.5）。
- **没有软/硬链接**：Linux 一个文件可以出现在多个目录里；MaxZOS 的条目只能挂在一条子链上。
- **最大相似点其实在这**：Linux 的内存文件系统 `tmpfs`（如 `/dev/shm`）和 MaxZOS 本质相同——内存里的一块文件系统，重启就没了。**MaxZOS 整个文件系统就是一块"永远为空的 tmpfs"**，记住这个类比，你就理解它一半了。

### 29.4 也有相同的地方

- **都是树形层级结构**：目录套目录，路径就是树的导航。
- **目录都是容器**：`mkdir` 的语义、`cd` 的语义、`..` 回到父目录——三者完全一致。
- **文件都是字节流**：不关心"里面是什么格式"，读出多少字节就是多少。
- **相对路径与绝对路径**：`/docs/note` 绝对、`docs/note` 相对当前目录——和 Linux 规则一模一样。

## 30. "万物皆文件"理论详解

### 30.1 这个理论从哪里来

"万物皆文件"（*Everything is a file*）是 **Unix 操作系统**的设计哲学，由 Ken Thompson、Dennis Ritchie 等人在 1970 年代提出。核心主张一句话：

> 不管底层是什么——磁盘文件、键盘、屏幕、打印机、网络连接、另一个进程——**都把它包装成"文件"**，用同一套操作去访问。

为什么？1970 年代的机器上设备五花八门，每个设备一个接口函数、一套手册，程序员根本记不住。统一成文件后，**学会一套接口，就能操作所有设备**。

### 30.2 核心思想：文件 = 字节流 + 统一操作

Unix 对"文件"的定义极度简化：

- **文件就是一段字节流**（byte stream），不关心内部结构；
- **操作就是固定几个**：打开 `open`、读 `read`、写 `write`、关 `close`。

任何资源只要能响应这组操作，就可以被当作文件使用。你不需要知道 `read` 背后读的是硬盘扇区还是键盘缓冲区——接口不变。

### 30.3 Linux 里"万物皆文件"长什么样

| "文件" | 实际是什么 | 怎么用 |
|---|---|---|
| `/dev/console` | 屏幕/键盘设备 | `echo hi > /dev/console` 就能在控制台显示 |
| `/proc/cpuinfo` | 内核现场生成的 CPU 信息 | `cat /proc/cpuinfo` 查看 |
| `log.txt \| grep error` | 管道（另一个进程的输出） | 当文件一样读 |
| socket | 网络连接 | `read`/`write` 收发数据 |
| `/dev/null` | 黑洞 | 写入即丢弃 |

它们唯一的共同点：**都用 `open`/`read`/`write`/`close` 操作**。工具因此可以任意组合——`grep`、`cat`、`sort` 不关心输入来自文件、管道还是设备。

### 30.4 MaxZOS 的"简化版万物皆文件"

MaxZOS 吸收了这条哲学的第一个切面：**目录与文件共用同一个结构体、同一张表、同一套接口**。

对照看：

| Unix 的"万物皆文件" | MaxZOS 的体现 |
|---|---|
| 设备、文件、目录统一抽象 | 目录与文件统一为 `file_t`（`fs.h`），`type` 字段区分 |
| 统一操作集合 | 建（`fs_create`/`fs_mkdir`）、查（`fs_read`/`fs_list`）、删（`fs_delete`）——对目录和文件是**同一批函数** |
| 遍历不需要知道类型 | `fs_list` 走一条子链，顺手输出名字和大小，不预先区分类型 |
| 读目录会报错 | `fs_read` 读目录返回 `FS_IS_DIR`——接口统一、行为按类型区分（与 Unix 的 `EISDIR` 异曲同工） |

具体来说：`fs_create("a", "...")` 和 `fs_mkdir("a")` 底层是**同一个** `fs_add_entry()`，只是 `type` 参数不同；`ls` 遍历时 `type` 只影响**展示**（目录名后加 `/`、大小列显示 `-`），不影响**遍历方式**。这就是"万物皆文件"的教学级浓缩。

### 30.5 好处与代价，以及还没做到什么

**好处**（在 MaxZOS 里也成立）：
- 学一次接口，到处能用——你只记 8 个 `fs_*` 函数就能操作整个文件系统；
- 数据组织统一——一张表、两条链，画一张图就能讲完整套结构；
- 将来扩展时思路清晰——新"东西"只要变成 `file_t` 的一个条目，就能被 `ls` 看到、被 `cat` 读取。

**还没做到什么**（也是继续努力的方向）：
- 键盘、屏幕、ACPI 电源管理**还不是文件**——`main.c` 直接轮询端口 `0x60`、直接写 `0xB8000`；
- 没有文件描述符（fd）概念——命令层直接调用 API，而不是拿到一个"打开的文件句柄"；
- 没有管道——`echo a | cat` 不成立。

将来如果想把它们变成文件：给键盘、屏幕各建一条"设备条目"（`type` 可以新增 `FS_TYPE_DEV`），`fs_read` 遇到设备条目就去读端口——`cat /dev/keyboard` 就能读按键了。这正是 `fs.h` 里"错误码追加在末尾"注释背后预留的扩展空间。

## 31. 代码地图：C 新手怎么读 fs/ 模块

### 31.1 正确的阅读顺序

别从头到尾硬啃。按这个顺序，每一步都有明确目标：

1. **先读 `fs.h` 全部**（59 行）：常量、错误码、`file_t` 结构体、8 个 API 声明。读完你就有"地图"了。
2. **再读 `fs.c` 从前往后**：内部函数在上、公开 API 在下，恰好是从"地基"到"使用"的顺序。
3. **`str.c` 扫一眼即可**：就是 `strlen`/`strcmp`/`strncmp`/`strcpy`/`itoa_dec` 五个自己实现的字符串函数，看名字就懂。

### 31.2 三个文件各管什么

| 文件 | 内容 | 你的角色 |
|---|---|---|
| `fs.h` | 常量、错误码、`file_t`、API 声明 | **签名冻结**：不轻易改 |
| `fs.c` | 静态表、两条链、路径解析、全部 API 实现 | 主战场：维护和扩展基本都在这里 |
| `str.c` | 字符串工具（自实现，不依赖 libc） | 只读即可，几乎不用动 |

### 31.3 file_t：一个结构体装下所有条目
...
想象一次典型操作后表的内容（`mkdir docs`、`create docs/note`、`create readme`）：

```
files[] 表（简化，省略未用的字段）：

 idx │ name    │ type  │ parent │ first_child │ next_sibling │ used
─────┼─────────┼───────┼────────┼─────────────┼──────────────┼─────
  0  │ "/"     │ 目录   │ 0(自指) │     1       │     -1       │  1
  1  │ docs    │ 目录   │ 0      │     2       │      3       │  1
  2  │ note    │ 文件   │ 1      │    -1       │     -1       │  1
...
```
根目录(0).first_child = 1
  └─> 1(docs).next_sibling = 3 ──> 3(readme).next_sibling = -1（链尾）
docs(1).first_child = 2 ──> 2(note).next_sibling = -1
```
`ls` 就是顺着 `first_child` 走进第一条链，一路 `next_sibling` 走到 `-1`。**一个条目只在它父目录的子链里出现一次**，所以 `ls` 不会重复列出。

**父链（垂直方向）**——从任意条目回根：
```
2(note).parent = 1 ──> 1(docs).parent = 0（根，停）
```
`fs_pwd` 就是沿这条链**反向**拼出绝对路径（`/docs/note`）；路径里的 `..` 上溯用的也是它。根目录 `parent` 自指，保证上溯到根必然停止。

### 31.5 路径解析走一遍（`cat /docs/note` 实例）

`fs_read("/docs/note", ...)` 首先调用 `fs_resolve()`，它会先调 `fs_resolve_path()`。逐轮追踪：

| 轮次 | 当前段 | 是最后一段吗 | 动作 |
|---|---|---|---|
| 第 0 轮 | `/` 开头 → 空段 | — | 跳过连续斜杠，`cur = 0`（根） |
| 第 1 轮 | `docs` | 否（后面还有 `/note`） | 在 `cur=0` 的子链里查 `docs` → 命中 idx 1，`cur = 1` |
| 第 2 轮 | `note` | 是 | 检查 `files[1]` 是目录 ✓，返回 `parent=1, name="note", namelen=4` |

然后 `fs_resolve()` 在 `1` 的子链里按名字找 `note` → 返回 idx 2。`fs_read` 拿到 idx 2，把 `content` 拷进调用者的缓冲。

注意两个细节：

- **零拷贝**：`name="note"` 没有复制，直接指向传入的路径字符串内部。所以段尾可能不是 `'\0'`——任何要"按名字做事"的代码都必须带上 `namelen`（对比 `fs_lookup_child` 的写法：`strncmp(..., namelen) == 0 && name[namelen] == '\0'`，先比长度再验边界）。
- **单一入口**：路径语义（`.`、`..`、连续斜杠、尾斜杠、绝对/相对）全部在这一个函数里，改路径行为只改它。

### 31.6 内部函数与公开 API 的分工

`fs.c` 里以 `static` 开头的都是**内部函数**（外部看不见，你可以随便改）：

| 内部函数 | 作用 |
|---|---|
| `fs_find_free` | 找空闲槽位（从 1 开始扫，0 永远留给根） |
| `fs_lookup_child` | 在某个目录的子链里按名字查找 |
| `fs_resolve_path` | **核心**：路径 →（父索引，末段名，末段长），零拷贝 |
| `fs_resolve` | 路径 → 条目索引；失败返回负数错误码（`-FS_XXX`） |
...
| API | 作用 | 典型失败 |
|---|---|---|
| `fs_init()` | 建根目录、重置当前目录（`kmain` 启动时调用） | — |
| `fs_create(path, content)` | 建文件（`content` 可为 `""`） | `FS_EXISTS` / `FS_FULL` |
| `fs_mkdir(path)` | 建目录 | `FS_EXISTS` / `FS_FULL` |
| `fs_read(path, out, maxlen)` | 读文件内容（自动补 `'\0'`） | `FS_IS_DIR` / `FS_NOT_FOUND` |
| `fs_delete(path)` | 删文件或**空**目录 | `FS_DIR_NOT_EMPTY` / `FS_IS_ROOT` / `FS_IS_CWD` |
...
都在 `fs.h` 顶部，改完 `make test` 必须全绿。详细教程见 **§18**，这里给速查：
...
- **表头** `Name	Size(Byte)`：在 `main.c` 的 `ls` 分支里直接写的；
- **每一行**：在 `fs.c` 的 `fs_list()` 里，三种写法：
  ```c
  out(files[c].name);          /* 先输出名字 */
  /* 目录：名字 + "/" + 制表符 + "-" */
  out("/\t-\n");
  /* 文件：制表符 + 十进制长度 + 换行 */
  out("\t"); itoa_dec(files[c].len, num); out(num); out("\n");
  ```

想改列宽、想让目录也显示大小、想加一列类型——都在这里动手。对齐用的制表位宽度是 `main.c` 的 `TAB_WIDTH`（现在为 8，改成 4 排版更紧凑）。

### 32.3 修改错误提示文字

错误码 → 提示消息的映射集中在 `main.c` 的 `fs_err_str()`，一个 `switch` 全在这：

| 错误码 | 当前提示 | 含义 |
|---|---|---|
| `FS_EXISTS` | `file already exists` | 同名条目已存在 |
...
| `FS_PATH_TOO_LONG` | `path too long` | 路径超过 128 字节缓冲 |
| `FS_DIR_NOT_EMPTY` | `directory not empty` | 删非空目录 |
| `FS_IS_ROOT` | `cannot remove root` | 删根目录 |
| `FS_IS_CWD` | `cannot remove current directory` | 删当前目录或其祖先 |

注意 `fs_delete` 的检查**顺序**（`fs.c`）：先查"是不是根"（`FS_IS_ROOT`），再查"是不是空目录"（`FS_DIR_NOT_EMPTY`），最后查"是不是当前目录/祖先"（`FS_IS_CWD`）。这就是为什么在 `/tmp` 里 `rm ..` 报 `cannot remove root` 而不是别的——顺序不同，同样的输入会报不同的错，改检查顺序前务必跑一遍 `make test`（测试里专门有几条用例锁死了这个行为）。

### 32.4 命令改名、加别名

命令匹配统一走 `main.c` 的 `cmd_is()`（前缀相等 + 边界检查：后面必须是空格、Tab 或行尾）。加别名就是加一个 `cmd_is` 条件：

```c
} else if (cmd_is("rm") || cmd_is("delete")) {
    /* delete 是旧名，rm 是新名，等价 */
```

已有先例：`rm` / `delete`、`clear` / `cls`。注意**不要**绕过 `cmd_is` 直接 `strncmp(input_buf, "rm", 2)`——那会让 `rmx` 也命中（§34.6）。

### 32.5 维护铁律（文件系统篇）

1. **不碰 `fs.h` 里已有的 API 签名**——8 个函数签名冻结，命令层全依赖它们；要加功能就**加**函数，不改旧的。
2. **错误码只追加在 `enum` 末尾**（`fs.h` 里有注释约定），不重排旧值。
3. **不在 `fs.c` 里写屏幕**——输出一律走回调（`fs_out_fn`）或返回错误码让 `main.c` 说话。
4. **路径解析只准走 `fs_resolve_path`**——别在自己代码里再写一遍"按 `/` 切段"。
5. **每个公开 API 要么成功、要么返回明确错误码**——不要静默失败、不要写一半返回。

### 32.6 改完怎么验证

- **自动化冒烟测试**：`make test`（依赖 `python3` 和 `qemu-system-i386`）。它启动无头 QEMU，用 QMP 注入按键、导出 `0xB8000` 的 VGA 文本内存，逐条比对屏幕输出。测试清单在 `tools/fs_test.py` 的 `tests` 列表里，**改完文件系统必跑**。
- **手工回归**：`make run` 打开窗口，按 §39.2 的文件系统清单过一遍（mkdir/cd/ls/cat/rm 的常规与边界路径）。
- **改完即测**：一次只改一个功能，改完立刻验证——这是本手册反复强调的习惯。

## 33. 如何正确添加新功能（决策树 + 套路）

### 33.1 第一步：决定改哪一层

添加功能前先回答"这功能属于哪一层"，避免在错误的地方动手：

| 你要做的事 | 层级 | 动哪些文件 | 要不要新错误码 |
|---|---|---|---|
| 加一个纯命令（如 `touch`、`pwd`、`help`） | 命令层 | 只动 `main.c` | 不要 |
| 加一个新的文件系统操作（如 `rename`、`mv`、`cp`） | 新接口 | `fs.h` + `fs.c` + `main.c` + `fs_test.py` | 可能要 |
| 修 bug、换存储后端、优化结构 | 内部实现 | 只动 `fs.c`（`fs.h` 不动） | 不要 |

判断口诀：**命令层是"换句话叫现有的 API"；新接口是"文件系统多了个能力"；内部是"同样的话换种说法"**。绝大多数需求都是命令层（情况 A）。

### 33.2 情况 A：命令层新功能（示例：`touch`）

目标：`touch <路径>` —— 创建一个空文件（若已存在则什么都不做）。`fs_create(path, "")` 恰好就是这个语义，所以**不需要动 `fs` 模块**。

步骤：

1. **找到 `main.c` 的 `process_command()`**，在 `else if` 链里挑个位置（比如 `about` 之前）插入：
   ```c
   } else if (cmd_is("touch")) {
       char* p = cmd_args();
       char* arg = next_arg(&p);
       if (arg == NULL) {
           terminal_write("usage: touch <path>\n");
       } else {
           fs_status_t st = fs_create(arg, "");
           if (st != FS_OK) terminal_write(fs_err_str(st));
       }
   }
   ```
2. **每个部分都是固定套路**：
   - `cmd_is("touch")`——命令名匹配（§32.4）；
   - `cmd_args()`——取命令名之后的参数起点；
   - `next_arg(&p)`——取第一个参数（自动支持双引号）；
   - 无参数 → 打印 `usage:` 提示；
   - 调 API → 非 `FS_OK` 就交给 `fs_err_str` 翻译。
3. **测试**：`make test` 前，往 `tools/fs_test.py` 的 `tests` 列表加两行（§33.6）。
4. 编译、跑 `make test`、`make run` 手工敲一遍。

这就是"新增第一个命令"的浓缩版——手把手全教程见 **§16**（含 `about` 命令的完整实战），带参数命令见 **§17**。

### 33.3 情况 B：新的文件系统接口

文件系统要提供**新能力**（而不是复用旧能力）时才走这条路。完整实战（`rename` 改名）见 **§19**，这里给流程骨架：

1. **`fs.h` 加声明**：`fs_status_t fs_xxx(const char* path);`——签名风格照抄现有 API。
2. **`fs.c` 实现**：优先复用内部函数（`fs_resolve` / `fs_resolve_path` / `fs_lookup_child` / `fs_add_entry`）。实现要点：**先全部校验、再动手写**（对照 `fs_delete` 的检查顺序），成功才修改状态（对照 `fs_cd` 的"解析成功才更新"）。
3. **新错误码**：需要的话追加在 `fs_status_t` 枚举末尾（§33.5）。
4. **`main.c` 接命令**：按 §33.2 的套路加分支；`fs_err_str` 补新错误码的提示。
5. **`fs_test.py` 加测试**（§33.6）。
6. **`make test` 全绿 + 手工回归**。

### 33.4 情况 C：内部实现重构 / 换存储后端

`fs.h` 一行不改，命令层无感知。注意这几点：

- **`fs_add_entry` 是全系统唯一的"写槽入口"**：新后端要改的写入路径集中在这里；
- 所有"找条目"的代码都要过 `fs_resolve` / `fs_lookup_child`——换数据结构（比如链表换哈希表）时只需改这两处；
- `fs_cwd` 是全局状态，任何改动保持"成功才更新"的原子性；
- `fs_init()` 里的挂载钩子注释就是为将来预留的：从磁盘/模块读数据后，用 `fs_add_entry` 逐条填充即可，API 与命令层零改动。
...
- **只能追加在末尾**，`fs.h` 里已有注释约定——重排会让旧值变含义（错误码将来可能被存储/协议使用）；
- **`fs_err_str` 补一个 `case`**：不补也行（有 `default` 兜底），但提示会变成 `unknown fs error`，不专业；
- 加了就顺手在 §32.3 的表格里补一行（本手册与代码保持同步）。

### 33.6 给回归测试加一行

`tools/fs_test.py` 的 `tests` 是一个（命令, 期望子串列表）的列表，每条命令执行前**自动清屏**，然后整屏搜索期望子串。加测试就是加一行：

```python
# 接 §33.2 的 touch 示例：
("touch newfile",       []),              # 期望无错误输出
("ls",                  ["newfile"]),     # 列表里能看到它
("touch newfile",       []),              # 幂等：已存在不报错
```

注意：

- 期望子串必须是**清屏后**整屏能搜到的内容（错误消息、文件名、提示符都行）；
- 命令文本里的双引号、上档字符有专门的映射（`KEYMAP`，如 `"` 写作 `shift-apostrophe`）；
- 失败时脚本会打印实际屏幕内容，照着比对就能定位。

### 33.7 上新功能后的检查清单

- [ ] 编译无新警告（`make kernel.elf`）
- [ ] `make test` 全绿
- [ ] 新错误码在 `enum` 末尾、`fs_err_str` 有 `case`
- [ ] 命令匹配用的 `cmd_is`，不是裸 `strncmp`
- [ ] 缺参数时有 `usage:` 提示
- [ ] 没动 `fs.h` 里任何已有签名
- [ ] `fs.c` 里没有直接写屏幕
- [ ] 更新了 §32.3 表格（如果加了错误码）和 README 的命令表（如果加了命令）
...
同理 `content[FS_MAX_CONTENT + 1]`——那 `+1` 不是白写的。

### 34.2 以为路径段是 '\0' 结尾的

路径解析是零拷贝的：`name_out` 指向路径字符串内部，段尾可能是 `'/'`。

```c
/* ❌ 错：strcpy 会把 "a/b" 整段拷走，连 '/' 一起 */
strcpy(files[i].name, name_out);

/* ✅ 对：按 namelen 逐字节拷贝（fs_add_entry 的写法） */
```

想检查"名字是否正好匹配"，看 `fs_lookup_child` 的双条件写法：`strncmp(..., namelen) == 0 && name[namelen] == '\0'`——先比长度，再验证段尾确实是结尾。

### 34.3 错误码的正负号

`fs_resolve()` 失败时返回的是**负数**错误码（`-FS_NOT_FOUND` 等），而公开 API 返回正数错误码。

```c
/* ❌ 错：拿正数错误码直接和 fs_resolve 的返回值比 */
int i = fs_resolve("nonexistent");
if (i == FS_NOT_FOUND) ...
...
`used` 是槽位的生命标志。删除时只改链指针、忘了 `used = 0`，槽位永远不会被复用，很快表就满了（`FS_FULL`）；创建时忘了 `used = 1`，`fs_find_free` 会把同一个槽发出去两次。
...
/* ✅ 对：输出走回调，或返回错误码让 main.c 输出 */
```

`fs_list` 的 `fs_out_fn` 参数就是为此存在的（§28.2 特点 7）。破坏这条，fs 模块就没法独立测试、将来也接不上串口。

### 34.6 命令匹配只准用 cmd_is

```c
/* ❌ 错：strncmp 只查前缀，clearxxx、clearb 全都会命中 clear */
if (strncmp(input_buf, "clear", 5) == 0) ...

/* ✅ 对：cmd_is 校验前缀 + 边界（空格/Tab/行尾），main.c 的现成工具 */
if (cmd_is("clear") || cmd_is("cls")) ...
```
